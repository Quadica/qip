...
- `AGENTS.md` This document. Defines your role, our business details, our coding standards, and how to interact with users
...
- **ACF Pro:** 6.4+ (currently 6.4.2)

## Infrastructure
- **Kinsta Hosting:** Managed WordPress with optimized performance
- **Testing Environment:** Staging site cloned as needed from our live site
...
- **GitHub Actions (Staging only):** CI/CD pipeline to Kinsta staging via SSH
- **Branch Triggers:** `main`
- **Automated Staging Deployment:** Push to `main` deploys to the cloned testing site
- **Production Deployment:** Manual upload and activation of tested plugin on live site (no automation)
...
+-- code-snippets/                       # Active PHP/JS snippets
...
- **Scope:** Staging only. Production has no automated deployments
- **Triggers:** push to `main` → deploys to staging
- **Process:** GitHub Actions builds and syncs plugin code to Kinsta over SSH. Dev dependencies are installed on staging so tests can run
- **Speed:** ~30 seconds from commit to staging deployment
- **Rollback (staging):** Use Kinsta full-site backup restore if needed; no custom rollback scripts

## **Production Deployment**
- **Manual Only:** Upload tested plugin build to live site and activate
- **Pre-Deploy:** Create a Kinsta full-site backup
- **Post-Deploy:** Verify admin/front-end steps; monitor error logs; disable any temporary debug logging
- **Rollback (production):** Restore from Kinsta backup if needed

## **Testing Workflow**
1. **Develop:** Write code and open PR if applicable
2. **Push:** `git push origin main`
3. **Deploy:** GitHub Actions deploys to staging automatically
4. **Self-Test (staging):** Run unit tests and light integration/smoke via SSH; fix and repeat until clean
5. **Handoff:** Notify the team; provide admin/front-end test steps and expected results
6. **Review:** You run acceptance tests and share feedback
7. **Iterate:** Address issues, redeploy to staging, and re-test
...
- Limited to Chromium and Firefox browsers

## **Task Management & Documentation:**
- **Always update TASKS.md** when completing any development task
- Mark tasks as completed [✅] with proper checkboxes
- Detailed implementation information is stored in session history files, not in the TASKS.md document
- Session details can be found in `docs/project-history/` directory
- TASKS.md should contain only task lists and completion tracking
